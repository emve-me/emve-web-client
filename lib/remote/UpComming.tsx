...
const Card = ({ children, style }: { style?: React.CSSProperties, children: React.ReactNode }) =>
  (<div style={{
      flex: 1,
      marginBottom: 24,
      backgroundColor: '#fff',
...
export default ({ channel }: TProps) => <ChannelController
  ...
  return <>
      ...
      <Card>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
          {nowPlaying ? <>
            ...
            <div style={{ padding:'0px 0 16px 0',fontSize: 16, color: '#444', fontWeight: 600 }}>Now playing</div>
            <Track {...nowPlaying} /></> : false}

          {upComing.length > 0 ?  <div style={{ padding:'0px 0 16px 0',fontSize: 16, color: '#444', fontWeight: 600 }}>Coming up</div>:false}
            {upComing.map(({ node }) =>
            <Track
    ...
    <Card style={{ justifyContent: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>

      <div><img style={{ borderRadius: 55, width: 55, border: 'solid 2px #ff2f7f' }} src={owner.picture}/></div>

      <div style={{ lineHeight: '175%', textAlign: 'center' }}>
        <div style={{ padding: '0px 0', fontWeight: 400, fontSize: 18 }}>Welcome to {owner.firstName}'s party!</div>

        <div>
          Pairing code:{' '}
          <span>{channel}</span>
        </div>
        <div><a href="">Invite guests</a></div>
      </div>
    </Card>
  </>
